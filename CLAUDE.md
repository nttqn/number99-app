...
**A real failure this caught (2026-09-07):** the very first live run (real Apple secrets, real account) failed the `archive` step with `error: Communication with Apple failed: Your team has no devices from which to generate a provisioning profile` + `No profiles for 'com.nttqn.number99' were found: ... iOS App Development provisioning profiles`. The tell is "App **Development**" — with only `CODE_SIGN_STYLE=Automatic` and `DEVELOPMENT_TEAM` set on the command line, `xcodebuild archive` defaulted to requesting a *Development* profile (which needs registered device UDIDs) instead of the *Distribution* one an App Store archive actually needs (which doesn't need any devices registered at all — that's what made the actual mismatch identifiable from the error text). Fix: also pass `CODE_SIGN_IDENTITY="Apple Distribution"` on the `archive` invocation, forcing it to resolve/create a Distribution certificate + profile instead. Not yet confirmed whether this fully resolves it or whether Archive succeeds but `-exportArchive` hits its own issue next — if a similar "wrong profile type" error recurs, check `CODE_SIGN_IDENTITY` on the export step too.
