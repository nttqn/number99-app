...
**Still not done, for a future pass**: no Game Center capability confirmation beyond what's set in the Apple Developer portal App ID (no code-level check that it's actually enabled); no App Store Connect app-record automation (the user creates that by hand); this has **never been run successfully end-to-end** — the archive/export/signing steps are new and unverified against a real Apple account, so expect at least one round of CI-log debugging the first time this actually runs (the Android signing setup needed exactly that on dino-egg-shooter — see [[feedback_release_signing_setup]] for the shape that debugging tends to take).
